...
def set_paths():
    ...
    """
    module_dir = os.path.abspath(__file__).split('sciligpy_utils')[0]
    ...
    return reactionlist_path, rateconstantlist_path, compositionlist_path, module_dir
...
def build_rates_list_kexp(rateconstlist, reactionlist, speciesindices):
    """ This function writes the list of rate expressions for each reaction (full expression for k).
    ...
    rates_list = []
    for i, line in enumerate(kmatrix):
        rate = 'rate[%s] = A%s * T**n%s * np.exp(-1*E%s/R/T) ' %(i, i, i, i)
        concentrations = ''
        for entry in reactant_dict[i]:
            if entry == 'n':   # if there is no reaction
                concentrations = '* 0'
